...
class QRCodeApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.qr_code_label = QLabel(self)  # To display the QR code image
        ...
        # Container widget for QR code with centered layout
        qr_code_container = QWidget()
        qr_code_layout = QVBoxLayout()
        qr_code_layout.setAlignment(Qt.AlignCenter)  # Center the QR code in the layout
        qr_code_container.setLayout(qr_code_layout)
        qr_code_layout.addWidget(self.qr_code_label)  # Add the QR code label to the new layout
        vbox.addWidget(qr_code_container)  # Add the container widget to the main layout
        ...
        self.qr_code_label.setAlignment(Qt.AlignCenter)  # Align the QR code in the center
        ...
        # Enable Enter key to trigger QR code generation
        self.input_field.returnPressed.connect(self.generate_qr_code)
    ...
    def generate_qr_code(self):
        ...
        files = [f for f in os.listdir(img_folder) if re.match(r'qrcode(\d+)\.(png|svg)', f)]
        numbers = [int(re.match(r'qrcode(\d+)\.(png|svg)', f).group(1)) for f in files]
        next_number = max(numbers, default=0) + 1
        file_number = f'{next_number:02d}'

        # Save as PNG
        png_path = os.path.join(img_folder, f"qrcode{file_number}.png")
        ...
        # Display the generated QR code on the label
        self.qr_code_label.setPixmap(pixmap)
        self.qr_code_label.setFixedSize(pixmap.size())

        # Save as SVG
        svg_path = os.path.join(img_folder, f"qrcode{file_number}.svg")
        ...
        # Update status
        self.status_label.setText(f"QR Code generated and saved as PNG and SVG as qrcode{file_number} in {img_folder}.")
